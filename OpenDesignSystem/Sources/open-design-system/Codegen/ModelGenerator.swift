...
public class ModelGenerator {
    ...
    private let designSystem: OpenDesignSystem

    private let jsonPath: String = ""

    public init(designSystem: OpenDesignSystem) {
        self.designSystem = designSystem
    }

    public init?(jsonPath: String, outputPath: String) {
        do {
            let contents = try Data(contentsOf: URL(fileURLWithPath: jsonPath))
            designSystem = try JSONDecoder().decode(OpenDesignSystem.self, from: contents)
            generate(outputPath: outputPath)
        } catch {
...
//        output += generateSurface() + "\n\n"
        ...
        print("---Generate---")
        do {
            try output.write(toFile: outputPath + "/Generated.swift", atomically: true, encoding: .utf8)
        } catch {
